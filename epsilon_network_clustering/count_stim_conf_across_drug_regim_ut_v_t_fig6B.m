%% Load untreated vs treated cluster results

load('metric_cluster_array_perstim_ut_v_t_052324.mat')
...
for file_num=1:pic_num_each_column% ;%len6
    ...
    for k=1:pic_num_each_row
        ...
        for i_col = 1:length(b)
            % If the cell contains more than one element
            if numel(b{i_col}) >= 1
                for i_row = 1:length(b{i_col})
                    A_sum(b{i_col}(i_row), i_col) = 1 + A_sum(b{i_col}(i_row), i_col);
                end
            end
        end
...
%exportgraphics(gcf, 'Fig6/Fig6B_0520.eps','Resolution',600)
saveas(gcf, strcat(fig_path, 'ut_v_t_stimuli_confusion_map','png'))
%close